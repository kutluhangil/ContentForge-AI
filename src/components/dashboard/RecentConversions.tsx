  ...
  if (conversions.length === 0) {
    return (
      <div className="text-center py-12 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-xl)]">
        <p className="text-[var(--text-tertiary)] text-sm">{td('no_conversions')}</p>
        <Link
        ...
        >
          {td('start_btn')} <ArrowRight size={14} />
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      {conversions.map((c, i) => {
        ...
        const statusVariant = statusVariants[c.status] ?? 'default';
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[var(--text-primary)] truncate">
                  {c.title ?? `${c.source_type.replace('_', ' ')} ${th('conversion_fallback')}`}
                </p>
                <div className="flex items-center gap-2 mt-0.5">
                  {c.outputs.slice(0, 3).map((o) => (
                    <span key={o.format} className="text-xs text-[var(--text-tertiary)]">
                      {formatLabels[o.format] ?? th('blog_summary')}
                    </span>
                  ))}
                  {c.outputs.length > 3 && (
              ...
              <div className="flex items-center gap-3 shrink-0">
                <Badge variant={statusVariant}>
                  ...
                  />
                  {ts(c.status as Parameters<typeof ts>[0]) ?? c.status}
                </Badge>
